...
const LayersManager = require('../phase1/layersManager');
const BasicTitles = require('../phase1/basicTitles');
const AudioEffects = require('../phase1/audioEffects');
const CropPan = require('../phase1/cropPan');

describe('Phase 1.2 - New Essential Editing Tools', () => {
  describe('LayersManager', () => {
    ...
    test('should get layers ordered by z-index', () => {
      layersManager.addLayer({ source: '/layer1.png' });
      layersManager.addLayer({ source: '/layer2.png' });
      layersManager.addLayer({ source: '/layer3.png' });
    ...
    test('should remove a layer', () => {
      const layer = layersManager.addLayer({ source: '/test.png' });
      layersManager.removeLayer(layer.id);
      
      expect(layersManager.getLayers()).toHaveLength(0);
    });

    test('should update layer properties', () => {
      const layer = layersManager.addLayer({ source: '/test.png', opacity: 1.0 });
      layersManager.updateLayer(layer.id, { opacity: 0.5, x: 200 });
      
      const updated = layersManager.getLayers()[0];
      expect(updated.opacity).toBe(0.5);
      expect(updated.position.x).toBe(200);
    });

    test('should move layer up/down', () => {
      const layer1 = layersManager.addLayer({ source: '/layer1.png' });
      const layer2 = layersManager.addLayer({ source: '/layer2.png' });
      
      layersManager.moveLayerUp(layer1.id);
      const layers = layersManager.getLayers();
      expect(layers[1].id).toBe(layer1.id);
    });

    test('should get blend modes', () => {
      const modes = layersManager.getBlendModes();
      expect(modes).toContain('normal');
      expect(modes).toContain('multiply');
      expect(modes).toContain('screen');
    });

    test('should export/import config', () => {
      layersManager.addLayer({ source: '/test.png', opacity: 0.7 });
      const config = layersManager.exportConfig();
      
      layersManager.clearLayers();
      expect(layersManager.getLayers()).toHaveLength(0);
      
      layersManager.importConfig(config);
      expect(layersManager.getLayers()).toHaveLength(1);
    });

    test('should throw error for non-existent layer', () => {
      expect(() => layersManager.removeLayer('invalid-id')).toThrow();
    });
  });

  describe('BasicTitles', () => {
    let basicTitles;
    ...
    test('should create instance', () => {
      expect(basicTitles).toBeDefined();
    });

    test('should get available styles', () => {
      const styles = basicTitles.getStyles();
      expect(styles).toHaveProperty('heading');
      expect(styles).toHaveProperty('subheading');
      expect(styles).toHaveProperty('body');
      expect(styles).toHaveProperty('caption');
    });

    test('should get available positions', () => {
      const positions = basicTitles.getPositions();
      expect(positions).toHaveProperty('topLeft');
      expect(positions).toHaveProperty('center');
      expect(positions).toHaveProperty('bottomCenter');
    });

    test('should add a title', () => {
      const title = basicTitles.addTitle({
        text: 'Hello World',
        fontSize: 64,
        fontColor: '#ff0000'
      });
    ...
    test('should apply predefined style', () => {
      const title = basicTitles.addTitle({
        text: 'Heading',
        style: 'heading'
      });
      ...
      expect(title.shadow.enabled).toBe(true);
    });

    test('should update title properties', () => {
      const title = basicTitles.addTitle({ text: 'Original' });
      basicTitles.updateTitle(title.id, { text: 'Updated', fontSize: 100 });
      
      const updated = basicTitles.getTitles()[0];
      expect(updated.text).toBe('Updated');
      expect(updated.fontSize).toBe(100);
    });

    test('should remove a title', () => {
      const title = basicTitles.addTitle({ text: 'Test' });
      basicTitles.removeTitle(title.id);
      
      expect(basicTitles.getTitles()).toHaveLength(0);
    });

    test('should clear all titles', () => {
      basicTitles.addTitle({ text: 'Title 1' });
      basicTitles.addTitle({ text: 'Title 2' });
      basicTitles.clearTitles();
      
      expect(basicTitles.getTitles()).toHaveLength(0);
    });

    test('should throw error for invalid title id', () => {
      expect(() => basicTitles.removeTitle('invalid-id')).toThrow();
    });
  });
  ...
  describe('AudioEffects', () => {
    ...
    test('should create instance', () => {
      expect(audioEffects).toBeDefined();
    });

    test('should get EQ presets', () => {
      const presets = audioEffects.getEQPresets();
      expect(presets.length).toBeGreaterThan(0);
      expect(presets.find(p => p.id === 'voice')).toBeDefined();
      expect(presets.find(p => p.id === 'music')).toBeDefined();
    });

    test('should get denoise levels', () => {
      const levels = audioEffects.getDenoiseLevels();
      expect(levels.length).toBe(3);
      expect(levels.find(l => l.id === 'light')).toBeDefined();
      expect(levels.find(l => l.id === 'medium')).toBeDefined();
      expect(levels.find(l => l.id === 'heavy')).toBeDefined();
    });

    test('should have eqPresets property', () => {
      expect(audioEffects.eqPresets).toBeDefined();
      expect(audioEffects.eqPresets.voice).toBeDefined();
      expect(audioEffects.eqPresets.voice.filters).toBeDefined();
    });

    test('should have denoiseLevels property', () => {
      expect(audioEffects.denoiseLevels).toBeDefined();
      expect(audioEffects.denoiseLevels.medium.nf).toBe(-35);
    });

    test('should throw error for invalid EQ preset', async () => {
      await expect(audioEffects.equalize('/input.mp4', '/output.mp4', 'invalid'))
        .rejects.toThrow();
    });

    test('should throw error for fade without parameters', async () => {
      await expect(audioEffects.fade('/input.mp4', '/output.mp4', {}))
        .rejects.toThrow();
    });

    test('should throw error for empty effect chain', async () => {
      await expect(audioEffects.applyChain('/input.mp4', '/output.mp4', []))
        .rejects.toThrow();
    });
  });
  ...
  describe('CropPan', () => {
    ...
    test('should create instance', () => {
      expect(cropPan).toBeDefined();
    });

    test('should get aspect ratios', () => {
      const ratios = cropPan.getAspectRatios();
      expect(ratios.length).toBeGreaterThan(0);
      expect(ratios.find(r => r.id === '16:9')).toBeDefined();
      expect(ratios.find(r => r.id === '9:16')).toBeDefined();
      expect(ratios.find(r => r.id === '1:1')).toBeDefined();
    });

    test('should get animation presets', () => {
      const presets = cropPan.getAnimationPresets();
      expect(presets.length).toBeGreaterThan(0);
      expect(presets.find(p => p.id === 'zoomIn')).toBeDefined();
      expect(presets.find(p => p.id === 'kenBurns')).toBeDefined();
    });

    test('should have aspectRatios property', () => {
      expect(cropPan.aspectRatios).toBeDefined();
      expect(cropPan.aspectRatios['16:9']).toBeDefined();
      expect(cropPan.aspectRatios['16:9'].width).toBe(16);
      expect(cropPan.aspectRatios['16:9'].height).toBe(9);
    });

    test('should have animationPresets property', () => {
      expect(cropPan.animationPresets).toBeDefined();
      expect(cropPan.animationPresets.zoomIn.startScale).toBe(1.0);
      expect(cropPan.animationPresets.zoomIn.endScale).toBe(1.3);
    });

    test('should throw error for crop without parameters', async () => {
      await expect(cropPan.crop('/input.mp4', '/output.mp4', {}))
        .rejects.toThrow();
    });

    test('should throw error for invalid aspect ratio in reframe', async () => {
      await expect(cropPan.reframe('/input.mp4', '/output.mp4', 'invalid'))
        .rejects.toThrow();
    });

    test('should throw error for invalid animation preset', async () => {
      await expect(cropPan.applyPreset('/input.mp4', '/output.mp4', 'invalid'))
        .rejects.toThrow();
    });
  });
});
